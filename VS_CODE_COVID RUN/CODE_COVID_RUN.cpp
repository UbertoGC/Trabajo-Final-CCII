...
void dibuja(ALLEGRO_FONT* main_font) {
    ...
    al_clear_to_color(a.sistema.fondo);
    al_draw_bitmap(a.img, 0, 0, 0);
...
void juego(ALLEGRO_FONT* main_font) {
    ...
    while (repetir)
    {
        ...
        if (dibujar && al_event_queue_is_empty(a.sistema.Mis_eventos))
        {
            dibuja(main_font);
            dibujar = false;
        }
        // esperamos a que ocurra un evento
        al_wait_for_event(a.sistema.Mis_eventos, &evento);
...
int main()
{
    ...
    a.img = al_load_bitmap("escenario.png");
    a.sistema.fondo = al_map_rgb(255,255,255);
    a.sistema.FPS = 60;
    timer = al_create_timer(1.0 / a.sistema.FPS);
    a.sistema.Mis_eventos = al_create_event_queue();
    //CREO LISTA DE EVENTOS
    a.sistema.Mis_eventos = al_create_event_queue();
    //ASIGNAR EVENTOS A LA LISTA
    al_register_event_source(a.sistema.Mis_eventos, al_get_keyboard_event_source());
    al_register_event_source(a.sistema.Mis_eventos, al_get_display_event_source(ventana));
    al_register_event_source(a.sistema.Mis_eventos, al_get_timer_event_source(timer));
